...
# Function to get the quantity and P&L percentage of CE and PE positions
def qty_positions_by_type(positions_net, CE_symbol, PE_symbol, positions_df):
    ...
    # Create avg_price in the DataFrame for later calculations
    positions_df['avg_price'] = positions_df.apply(
    ...
    for position in positions_net:
        ...
        if not matching_row.empty:
            avg_price = matching_row['avg_price'].values[0]  # Get avg_price for this position

            if position['tradingsymbol'] == CE_symbol:
                if avg_price != 0:  # Check for division by zero
                    qty_CE += int(abs(position['quantity']) / 25)
                    CE_PLPREC = int((position['last_price'] - avg_price) / avg_price * 100)
                    
            elif position['tradingsymbol'] == PE_symbol:
                if avg_price != 0:  # Check for division by zero
                    qty_PE += int(abs(position['quantity']) / 25)
                    PE_PLPREC = int((position['last_price'] - avg_price) / avg_price * 100)
    ...
    try:
        ...
        with open('output.txt', 'w') as file:
            ...
            try:
                ...
                qty_CE, qty_PE, CE_PLPREC, PE_PLPREC = qty_positions_by_type(positions_net, CE_symbol, PE_symbol)
                ...
                logging.error(f"{str(e)} error occurred")
    except Exception as e:
