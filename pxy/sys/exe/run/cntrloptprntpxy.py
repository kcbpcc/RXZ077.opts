...
print_df['MN'] = np.where(print_df['product'] == 'NRML', '⌛', '🔢')
...
output_lines.append(
    ...
    right_aligned_format.format(
        ...
        f"A/C-tPnL:{BRIGHT_GREEN}{str(round(CnC_tCap_rounded + (total_invested_all / 100000) + cashround - 1.0, 2)).zfill(5)}{RESET}"
    )
)
...
for group, data in grouped_df:
    ...
    if total_invested_group != 0:
        ...
        value_statement = f"  {pe_count:02d} -🟥- {int(total_invested_pe):06d}  {Istrike}   {int(total_invested_ce):06d}  -🟩- {ce_count:02d}"
